...
vmgStep state =
    let
        vmgValue =
            getVmgValue state.windAngle state.velocity

        upwindVmg =
            getUpwindVmg state.windSpeed

        downwindVmg =
            getDownwindVmg state.windSpeed
    in
        { state
            | vmgValue = vmgValue
            , upwindVmg = upwindVmg
...
findVmgInInterval windSpeed minAngle maxAngle =
    let
        vmgs =
            L.map (makeVmg windSpeed) (floatRange minAngle maxAngle)

        bestMaybe =
            L.sortBy .value vmgs |> L.reverse |> L.head
    in
