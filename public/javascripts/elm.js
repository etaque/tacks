...
Elm.Views.Home.make = function (_elm) {
   ...
   var raceCourseBlock = F2(function (t,
   _v0) {
      return function () {
         return $Graphics$Input.clickable($Inputs.navigate($State.Show(_v0)))($Graphics$Element.centered(A2($Views$Utils.raceCourseName,
         t,
         _v0.raceCourse)));
      }();
   });
   ...
   _v2) {
      return function () {
         ...
         A2($List.map,
         raceCourseBlock(t),
         _v2.liveCenterState.courses));
      }();
   });
   _elm.Views.Home.values = {_op: _op
                            ,view: view
                            ,raceCourseBlock: raceCourseBlock};
   return _elm.Views.Home.values;
};
Elm.Views = Elm.Views || {};
         ...
         {case "_Tuple2":
            return function () {
                 ...
                 {case "Home":
                    return A2($Graphics$Element.flow,
                      $Graphics$Element.down,
                      _L.fromArray([A3($Views$TopBar.view,
                                   t,
                                   _v0._0,
                                   appState)
                                   ,A3($Views$Home.view,
                    ...
                    return A2($Graphics$Element.flow,
                      ...
                      _L.fromArray([A3($Views$TopBar.view,
                                   ...
                                   _v0._0,
                                   appState)
                                   ,A4($Views$ShowRaceCourse.view,
                                   t,
                                   _v0._0,
                                   appState,
                                   _v4._0)]));}
                 _U.badCase($moduleName,
                 "between lines 17 and 43");
              }();}
         _U.badCase($moduleName,
         "between lines 17 and 43");
      }();
...
Elm.Views.ShowRaceCourse.make = function (_elm) {
   ...
   t) {
      return $Graphics$Input.clickable($Inputs.navigate($State.Play(raceCourse)))($Graphics$Element.centered($Text.fromString(t("join"))));
   });
   var view = F4(function (t,
   width,
   appState,
   _v0) {
      return function () {
         return A2($Graphics$Element.flow,
         $Graphics$Element.down,
         _L.fromArray([$Graphics$Element.centered(A2($Views$Utils.raceCourseName,
                      t,
                      _v0.raceCourse))
                      ,A2(joinButton,
                      _v0.raceCourse,
...
Elm.Views.TopBar.make = function (_elm) {
   ...
   var height = 80;
   var logo = function () {
      var img = A3($Graphics$Element.image,
      logoWidth,
      height,
      "/assets/images/logo-header-2.png");
      var msg = $Inputs.navigate($State.Home);
...
Elm.Views.Utils.make = function (_elm) {
   ...
   _v0) {
      return function () {
         return $Text.fromString(t(A2($Basics._op["++"],
         "generators.",
         A2($Basics._op["++"],
         _v0.slug,
         ".name"))));
