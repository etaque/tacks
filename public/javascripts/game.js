...
Elm.ShiftMaster.make = function (_elm) {
   ...
   Native.Ports.incomingSignal(function (v) {
      return typeof v === "object" && "now" in v && "startTime" in v && "course" in v && "opponents" in v && "leaderboard" in v && "gusts" in v ? {_: {}
                                                                                                                                                  ...
                                                                                                                                                  })) : _E.raise("invalid input, expecting JSArray but got " + v.gusts)} : _E.raise("invalid input, expecting JSObject [\"now\",\"startTime\",\"course\",\"opponents\",\"leaderboard\",\"gusts\"] but got " + v);
   }));
...
Elm.Render.Race.make = function (_elm) {
   ...
   gust) {
      return Graphics.Collage.move(gust.position)(Graphics.Collage.alpha(0.5)(Graphics.Collage.filled(Color.black)(Graphics.Collage.circle(gust.radius))));
   });
   ...
   var renderWake = function (wake) {
      return function () {
         ...
         var renderWakePoint = function (_v2) {
            return function () {
               switch (_v2.ctor)
               {case "_Tuple2":
         ...
         var span = 5;
         var points = List.map(renderWakePoint)(List.filter(function (_v6) {
            return function () {
               switch (_v6.ctor)
               {case "_Tuple2":
   ...
   var renderRelative = function (_v14) {
      return function () {
         return function () {
            ...
            _v14.player,
            _v14.course.laps);
            var downwindOrStartLine = List.isEmpty(_v14.player.passedGates) ? A3(renderStartLine,
            _v14.course.downwind,
            _v14.course.markRadius,
            _U.cmp(_v14.countdown,
            0) < 1) : A3(renderGate,
            _v14.course.downwind,
            _v14.course.markRadius,
            _U.eq(nextGate,
            Maybe.Just(Game.Downwind)));
            var justForms = _L.fromArray([renderBounds(_v14.course.bounds)
                                         ,renderIslands(_v14)
                                         ,downwindOrStartLine
                                         ,A3(renderGate,
                                         _v14.course.upwind,
                                         ...
                                         ,renderPlayer(_v14.player)
                                         ,Graphics.Collage.group(A2(List.map,
                                         renderOpponent,
                                         _v14.opponents))
                                         ,renderGusts(_v14.wind)]);
            return Graphics.Collage.move(Geo.neg(_v14.player.center))(Graphics.Collage.group(_L.append(justForms,
